...
from models.mmd.network import GANLung
...
def train(opt, train_loader: DataLoader, test_loader: DataLoader) -> None:
    lungan = GANLung(opt)
    all_losses = {k: [] for k in ['err_g', 'err_d', 'err_g_con', 'err_g_adv']}
    ...
    for epoch in range(opt.epochs):
        epoch_g_loss, epoch_d_loss, epoch_g_con_loss, epoch_g_adv_loss, epoch_mmd_loss = 0, 0, 0, 0, 0 # Track total loss for the epoch
        ...
        for i, data in enumerate(train_loader):
            ...
            # Optimize parameters
            err_g, err_d, err_g_con, err_g_adv, mmd = lungan.optimize_params(input)
    ...
    # store_results(results, lungan.w_con, opt.seed)
