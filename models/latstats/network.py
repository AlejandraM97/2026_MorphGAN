...
from utils.losses import l2_loss, l2_z_loss, ssim_loss, combined_loss, LatentChannelStatsLoss
...
class GANLung(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.l_adv = l2_loss
        self.l_latstat = LatentChannelStatsLoss()
        if opt.con_loss == 'l2':
            self.l_con = l2_loss
    ...
    def backward_g(self, input, fake, latent_i, latent_o):
        """ Backpropagate through Generator using latent statistics loss """
        err_g_adv = self.l_adv(self.discriminator(input)[1], self.discriminator(fake)[1])
        err_g_con = self.l_con(fake, input)
        err_g_enc = self.l_enc(latent_o, latent_i)

        # Nueva regularización por estadísticas de canales
        err_g_latstat = self.l_latstat(latent_i, latent_o)

        err_g = (
            err_g_adv * self.w_adv +
            err_g_con * self.w_con +
            err_g_enc * self.w_enc +
            err_g_latstat * self.w_latstat
        )

        err_g.backward(retain_graph=True)

        # Devuelve la pérdida adicional en vez de mmd
        return err_g, err_g_con, err_g_adv, err_g_enc, err_g_latstat


        # 🔹 Now return mmd too for logging
